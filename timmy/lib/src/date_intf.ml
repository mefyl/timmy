...
module type V0_10_4 = sig
  ...
  (** [make ~year ~month ~day] is [{ day; month; year }] if it represents a
      valid date or a relevant error message otherwise. *)
  val make : year:int -> month:Month.t -> day:int -> (t, string) Result.t

  (** [make_overflow] is {!make} except out of range components will readjust
      the next component until the date becomes valid. This enables to perform
      simple arithmetics over dates like adding or substracting days and months
      without worrying about month and year boundaries and readjust the final
      results.
   ...
   ~day:1 ()]
      is [{year = 2020; month = 11; day = 1}] and
      [make
  ...
  (** [of_tuple (year, month, day)] is the corresponding date if it is valid or
      a relevant error message otherwise. *)
