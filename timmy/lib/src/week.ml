...
let _year week =
  ...
  match month with December when day >= 29 -> year + 1 | _ -> year

let make ~year n =
  let res = { n; year } in
  if n < 1 then Result.failf "week %i is less than 1" n
...
let days week =
  Base.Sequence.unfold
    ~init:(to_date week, 0)
    ~f:(function
      | _, 7 -> None | date, n -> Some (Date.(date + n), (date, n + 1)))
...
let of_string s =
  ...
  | [ year; week ] -> (
    try make ~year:(Int.of_string year) (Int.of_string week)
    with _ -> Result.failf "invalid number in date: %S" s)
